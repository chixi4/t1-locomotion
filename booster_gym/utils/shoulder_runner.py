...
class FrozenLegShoulderRunner:
    ...
    def __init__(self):
        ...
        expected_arm_action_dim = int(self.cfg["control"].get("expected_arm_action_dim", self.arm_action_dim))
        expected_leg_action_dim = int(self.cfg["control"].get("expected_leg_action_dim", 12))
        if self.arm_action_dim != expected_arm_action_dim or self.leg_action_dim != expected_leg_action_dim:
            raise ValueError(
                f"Expected {expected_arm_action_dim} upper-body and {expected_leg_action_dim} leg DOFs, "
                f"got {self.arm_action_dim} and {self.leg_action_dim}"
            )

        self.learning_rate = self.cfg["algorithm"]["learning_rate"]
        self.actor_mean_scale = self._actor_mean_scale()
        self.model = ActorCritic(
            self.arm_action_dim,
            self.env.num_obs,
            self.env.num_privileged_obs,
            logstd_init=float(self.cfg["algorithm"].get("logstd_init", -2.5)),
            actor_mean_scale=self.actor_mean_scale,
    ...
    def _actor_mean_scale(self):
        by_dof = self.cfg["algorithm"].get("actor_mean_scale_by_dof")
        if by_dof:
            scales = []
            for dof_idx in self.env.arm_indices.tolist():
                name = self.env.dof_names[dof_idx]
                if name not in by_dof:
                    raise KeyError(f"actor_mean_scale_by_dof missing {name}")
                scales.append(float(by_dof[name]))
            print(f"Using per-DOF actor mean scale: {dict(zip([self.env.dof_names[i] for i in self.env.arm_indices.tolist()], scales))}")
            return scales
                ...
                self.recorder.save(
                    {
                        ...
                        "actor_mean_scale": self.actor_mean_scale,
                        "actor_mean_scale_by_dof": self.cfg["algorithm"].get("actor_mean_scale_by_dof"),
                        "logstd_min": self.cfg["algorithm"].get("logstd_min"),
                        "logstd_max": self.cfg["algorithm"].get("logstd_max"),
