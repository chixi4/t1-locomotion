...
run_id="2026-05-02-11-47-30"
log="artifacts/autoresearch/official_baseline_${run_id}.log"
manifest="artifacts/autoresearch/official_baseline_${run_id}.json"
exec > "$log" 2>&1
start_iso=$(date -Is)
...
echo "COMMAND=/opt/conda/bin/python -u train.py --task T1 --headless true --num_envs 4096 --max_iterations 500"
set +e
/opt/conda/bin/python -u train.py --task T1 --headless true --num_envs 4096 --max_iterations 500
rc=$?
set -e
finish_iso=$(date -Is)
...
START_ISO="$start_iso" FINISH_ISO="$finish_iso" RC="$rc" COMMIT="$commit" LATEST_RUN="$latest_run" LATEST_CKPT="$latest_ckpt" LOG_PATH="$log" MANIFEST="$manifest" /opt/conda/bin/python - <<'PY'
import json, os
manifest = {
  ...
  "command": ["/opt/conda/bin/python", "-u", "train.py", "--task", "T1", "--headless", "true", "--num_envs", "4096", "--max_iterations", "500"],
  "num_envs": 4096,
  "max_iterations": 500,
  "wandb_mode": "disabled",
