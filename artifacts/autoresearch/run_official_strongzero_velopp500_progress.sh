...
LABEL="shoulder4_official_strongzero_velopp500"
DISPLAY_NAME="T1Shoulder4 Official StrongZero VelOpp model_500"
...
COMMAND_TEXT="OfficialStrongZeroVelOpp: checkpoint=null; official leg rewards/settings restored; shoulder_static_posture targets exact zero at stand; pitch target from abs(vx_cmd)*left-right foot x distance; side roll uses commanded outward foot velocity so right foot moving outward on right sidestep raises left arm and left foot moving outward on left sidestep raises right arm."
LEG_CKPT="${LEG_CKPT:-logs/2026-05-05-11-09-07/nn/model_4000.pth}"
FINAL_ITER="${FINAL_ITER:-500}"
...
META_JSON="artifacts/autoresearch/official_strongzero_velopp_runs.json"
...
echo "CONFIG reward: strong zero stand target; pitch abs(cmd_x)*foot_sag; stronger commanded outward foot velocity side roll right-foot->left-arm left-foot->right-arm" | tee -a "${TRAIN_LOG}"
phase_start train
...
if ! tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" "${OUT_DIR}" "${METRICS_JSON}" "${TRAIN_LOG}" "envs/${TASK}.yaml" "envs/t1.py" "envs/__init__.py" "utils/shoulder_runner.py" "utils/model.py" "${URDF_FILE}" "${META_JSON}" "artifacts/autoresearch/run_official_strongzero_velopp500_progress.sh" "artifacts/autoresearch/run_shoulder4_eval_pair.sh" "artifacts/autoresearch/eval_shoulder4_frozen.py" "artifacts/autoresearch/export_shoulder_metrics.py"; then fail_run pack "tar packaging failed"; fi
phase_done pack
