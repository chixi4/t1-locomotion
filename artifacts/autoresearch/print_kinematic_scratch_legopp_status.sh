...
PROC="$(ps -eo pid,etime,cmd | grep -E 'train_shoulder4_frozen.py.*KinematicScratchLegOpp|run_kinematic_scratch_legopp300_progress.sh' | grep -v grep | sed -n '1,3p')"
SCALARS=""
if [ -n "$LOG" ]; then
...
cat <<OUT
T1 Kinematic Scratch LegOpp Progress Dashboard
======================================================================
Run:   T1Shoulder4 Kinematic Scratch LegOpp model_300
...
Side:  cross-side roll with signed delta lock: right leg outward raises left arm; left leg raises right
