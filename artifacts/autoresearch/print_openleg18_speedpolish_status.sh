...
if [[ -f "${DONE_JSON}" ]]; then
  ...
  PHASE="failed"
elif ps -eo cmd | grep -E 'run_openleg18_speedpolish200_progress.sh|train_fullbody_residual.py.*T1Upper9CameraStableOfficialOpenLeg18LegResidualSpeedPolish' | grep -v grep >/dev/null; then
  PHASE="bc/train"
fi
