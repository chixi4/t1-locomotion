...
TASK="T1Upper9CameraStableOfficialOpenLeg18LegResidualBalanceCap_train300"
LABEL="upper9_camera_stable_openleg18_legresidual_balancecap300"
FINAL_ITER="300"
...
if [[ -f "${DONE_JSON}" ]]; then
  ...
  PHASE="failed"
elif ps -eo cmd | grep -E 'run_openleg18_balancecap300_progress.sh|train_fullbody_residual.py.*T1Upper9CameraStableOfficialOpenLeg18LegResidualBalanceCap' | grep -v grep >/dev/null; then
  PHASE="bc/train"
fi
