...
FINAL_ITER="400"
META_JSON="artifacts/autoresearch/nightk_runs.json"
MASTER_LOG="artifacts/autoresearch/nightk_400_full_auto_$(date +%Y%m%d_%H%M%S).log"
RUN_CODES=(k)
...
start_gpu_audit() {
  if [[ -x artifacts/autoresearch/nightk_gpu_audit_wsl.sh ]]; then
    mkdir -p artifacts/autoresearch/gpu_train_audit
    if ! pgrep -f "nightk_gpu_audit_wsl.sh" >/dev/null 2>&1; then
      nohup bash artifacts/autoresearch/nightk_gpu_audit_wsl.sh \
        > artifacts/autoresearch/gpu_train_audit/nightk_wsl_logger.out 2>&1 &
      echo "$!" > artifacts/autoresearch/gpu_train_audit/nightk_wsl_logger.pid
      echo "==== GPU audit started pid=$(cat artifacts/autoresearch/gpu_train_audit/nightk_wsl_logger.pid) ====" | tee -a "${MASTER_LOG}"
    else
...
run_one() (
  ...
  tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
    ...
    "${URDF_FILE}" \
    "${META_JSON}" \
    "artifacts/autoresearch/run_nightk_400_full_auto.sh" \
...
echo "==== NIGHTK START $(date '+%Y-%m-%d %H:%M:%S') order=${RUN_CODES[*]} ====" | tee -a "${MASTER_LOG}"
start_gpu_audit
...
echo "==== NIGHTK DONE $(date '+%Y-%m-%d %H:%M:%S') ====" | tee -a "${MASTER_LOG}"
