...
FINAL_ITER="400"
META_JSON="artifacts/autoresearch/nightf_runs.json"
MASTER_LOG="artifacts/autoresearch/nightf_400_full_auto_$(date +%Y%m%d_%H%M%S).log"
RUN_CODES=(f)
...
start_gpu_audit() {
  if [[ -x artifacts/autoresearch/nightf_gpu_audit_wsl.sh ]]; then
    mkdir -p artifacts/autoresearch/gpu_train_audit
    if ! pgrep -f "nightf_gpu_audit_wsl.sh" >/dev/null 2>&1; then
      nohup bash artifacts/autoresearch/nightf_gpu_audit_wsl.sh \
        > artifacts/autoresearch/gpu_train_audit/nightf_wsl_logger.out 2>&1 &
      echo "$!" > artifacts/autoresearch/gpu_train_audit/nightf_wsl_logger.pid
      echo "==== GPU audit started pid=$(cat artifacts/autoresearch/gpu_train_audit/nightf_wsl_logger.pid) ====" | tee -a "${MASTER_LOG}"
    else
...
run_one() (
  ...
  tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
    ...
    "${URDF_FILE}" \
    "${META_JSON}" \
    "artifacts/autoresearch/run_nightf_400_full_auto.sh" \
...
echo "==== NIGHTF START $(date '+%Y-%m-%d %H:%M:%S') order=${RUN_CODES[*]} ====" | tee -a "${MASTER_LOG}"
start_gpu_audit
...
echo "==== NIGHTF DONE $(date '+%Y-%m-%d %H:%M:%S') ====" | tee -a "${MASTER_LOG}"
