...
TASK="T1Shoulder4GaitPhaseNightN7PSideAmpLegLikeSagGate_from7000LegFrozen_train300"
LABEL="shoulder4_night_n7p_sideamp_leglikesaggate300"
DISPLAY_NAME="T1Shoulder4 NightN7P SideAmp LegLikeSagGate model_300"
NOTE="LegLikeClean rerun from SideAmp model_300. Restore shoulder_dynamic_target=-4.0 and gate static same-pitch residual out during sagittal commands."
COMMAND_TEXT="SagGate: warm-start SideAmp model_300; keep LegLikeClean 30s episodes and 8-12s command resampling; shoulder_dynamic_target -2.0 -> -4.0; static residual gate 0.15*(1-lateral_gate)*(1-pitch_gate)."
LEG_CKPT="${LEG_CKPT:-logs/2026-05-05-11-09-07/nn/model_4000.pth}"
ARM_INIT_CKPT="${ARM_INIT_CKPT:-logs/2026-05-13-10-37-59/nn/model_300.pth}"
...
META_JSON="artifacts/autoresearch/nightn7p_leglikesaggate_runs.json"
...
write_status_json() {
  ...
  "${PYTHON_BIN}" - "$target" "$status" "$failure_phase" "$failure_reason" "$LABEL" "$DISPLAY_NAME" "$TASK" "${RUN_DIR:-}" "${ARM_CKPT:-}" "$ARM_INIT_CKPT" "$LEG_CKPT" "$OUT_DIR" "$METRICS_JSON" "$WEB_TAR" "$TRAIN_LOG" <<'PY'
import json
import sys
from datetime import datetime, timezone
from pathlib import Path
keys = ["target", "status", "failure_phase", "failure_reason", "label", "display_name", "task", "run_dir", "arm_checkpoint", "arm_init_checkpoint", "leg_checkpoint", "eval_dir", "metrics_json", "web_inputs", "train_log"]
...
fail_run() {
  ...
  write_status_json "${FAIL_JSON}" failed "${failure_phase}" "${failure_reason}" | tee -a "${TRAIN_LOG}" || true
  exit 1
}

echo "START run label=${LABEL} task=${TASK} envs=${TRAIN_NUM_ENVS} final_iter=${FINAL_ITER} $(date '+%Y-%m-%d %H:%M:%S')" | tee "${TRAIN_LOG}"
echo "CONFIG warm_start=${ARM_INIT_CKPT} reset_optimizer=true episode=30s command_resample=8-12s" | tee -a "${TRAIN_LOG}"
echo "CONFIG patch: shoulder_dynamic_target -2.0 -> -4.0; static residual *= (1 - pitch_gate)" | tee -a "${TRAIN_LOG}"
echo "CONFIG side_amp preserved: pitch_amp=0.84 roll_base=0.0 roll_extra=0.46 roll_max=0.34 min_outward=0.10" | tee -a "${TRAIN_LOG}"

phase_start train
if ! stdbuf -oL -eL "${PYTHON_BIN}" train_shoulder4_frozen.py \
  --task "${TASK}" \
  --checkpoint "${ARM_INIT_CKPT}" \
  --leg_checkpoint "${LEG_CKPT}" \
  --num_envs "${TRAIN_NUM_ENVS}" \
  --max_iterations "${FINAL_ITER}" 2>&1 | tee -a "${TRAIN_LOG}"; then
...
if ! bash artifacts/autoresearch/run_shoulder4_eval_pair.sh \
  ...
  --fps "${EVAL_FPS}" \
  --clean 2>&1 | tee -a "${TRAIN_LOG}"; then
...
if ! "${PYTHON_BIN}" artifacts/autoresearch/export_shoulder_metrics.py \
  ...
  --checkpoint "${ARM_CKPT}" \
  --note "${NOTE} Command: ${COMMAND_TEXT}" 2>&1 | tee -a "${TRAIN_LOG}"; then
...
if ! tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
  "${OUT_DIR}" "${METRICS_JSON}" "${TRAIN_LOG}" \
  "envs/${TASK}.yaml" "envs/t1.py" "envs/__init__.py" \
  "utils/shoulder_runner.py" "utils/model.py" "${URDF_FILE}" "${META_JSON}" \
  "artifacts/autoresearch/run_n7p_leglikesaggate300_progress.sh" \
