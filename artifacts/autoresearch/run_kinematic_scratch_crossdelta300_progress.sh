...
TASK="T1Shoulder4KinematicScratchCrossDelta_from7000LegFrozen_train300"
LABEL="shoulder4_kinematic_scratch_crossdelta300"
DISPLAY_NAME="T1Shoulder4 Kinematic Scratch CrossDelta model_300"
NOTE="Scratch arm policy with one continuous kinematic motion reward: no explicit standing posture angle, no low-speed still threshold, dominant-foot cross-delta side arm lift."
COMMAND_TEXT="KinematicScratchCrossDelta: checkpoint=null; old shoulder style and arm energy rewards disabled; pitch target from abs(vx_cmd)*left-right foot x distance; side roll target dominant-foot delta coupled so only the opposite arm is raised by the more outward leg."
...
META_JSON="artifacts/autoresearch/kinematic_scratch_crossdelta_runs.json"
...
echo "CONFIG reward: pitch abs(cmd_x)*foot_sag; dominant cross-delta side roll right-leg->left-arm left-leg->right-arm" | tee -a "${TRAIN_LOG}"
phase_start train
...
if ! tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" "${OUT_DIR}" "${METRICS_JSON}" "${TRAIN_LOG}" "envs/${TASK}.yaml" "envs/t1.py" "envs/__init__.py" "utils/shoulder_runner.py" "utils/model.py" "${URDF_FILE}" "${META_JSON}" "artifacts/autoresearch/run_kinematic_scratch_crossdelta300_progress.sh" "artifacts/autoresearch/run_shoulder4_eval_pair.sh" "artifacts/autoresearch/eval_shoulder4_frozen.py" "artifacts/autoresearch/export_shoulder_metrics.py"; then fail_run pack "tar packaging failed"; fi
phase_done pack
