...
DISPLAY_NAME="T1Shoulder4 NightN7P SideAmp LegLikeClean model_300"
NOTE="LegLikeClean continuation from SideAmp model_300. 30s episodes, command resampling 8-12s, direct survival/tracking rewards zeroed; shoulder style terms are guardrails."
COMMAND_TEXT="LegLikeClean: warm-start SideAmp model_300; episode 10s -> 30s; command resampling 100s -> 8-12s; survival/tracking x/y/yaw rewards -> 0; shoulder style rewards reduced about half."
...
META_JSON="artifacts/autoresearch/nightn7p_leglikeclean_runs.json"
...
keys = [
    "target", "status", "failure_phase", "failure_reason", "label", "display_name", "task",
    "run_dir", "arm_checkpoint", "arm_init_checkpoint", "leg_checkpoint", "eval_dir", "metrics_json",
    "web_inputs", "train_log",
]
...
echo "CONFIG warm_start=${ARM_INIT_CKPT} reset_optimizer=true episode=30s command_resample=8-12s direct_tracking_rewards=0" | tee -a "${TRAIN_LOG}"
...
echo "CONFIG style guardrails: static=-2.6 dynamic=-2.0 boundary=-0.9 balance=-0.45" | tee -a "${TRAIN_LOG}"
...
if ! tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
  ...
  "${URDF_FILE}" \
  "${META_JSON}" \
  "artifacts/autoresearch/run_n7p_leglikeclean300_progress.sh" \
