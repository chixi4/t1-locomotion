...
TASK="T1Shoulder4KinematicScratchLegOpp_from7000LegFrozen_train300"
LABEL="shoulder4_kinematic_scratch_legopp300"
DISPLAY_NAME="T1Shoulder4 Kinematic Scratch LegOpp model_300"
NOTE="Scratch arm policy with one continuous kinematic motion reward plus a signed roll-delta lock: no explicit standing posture angle, no low-speed still threshold, dominant-foot lateral displacement raises only the opposite arm."
COMMAND_TEXT="KinematicScratchLegOpp: checkpoint=null; old shoulder style and arm energy rewards disabled; pitch target from abs(vx_cmd)*left-right foot x distance; side roll uses dominant lateral foot delta plus left_roll+right_roll lock so right leg outward -> left arm and left leg outward -> right arm."
...
META_JSON="artifacts/autoresearch/kinematic_scratch_legopp_runs.json"
...
echo "CONFIG scratch arm checkpoint=null; leg=${LEG_CKPT}; no static posture reward; no arm energy reward" | tee -a "${TRAIN_LOG}"
echo "CONFIG reward: pitch abs(cmd_x)*foot_sag; dominant cross-delta side roll plus signed roll-delta lock right-leg->left-arm left-leg->right-arm" | tee -a "${TRAIN_LOG}"
phase_start train
...
if ! tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" "${OUT_DIR}" "${METRICS_JSON}" "${TRAIN_LOG}" "envs/${TASK}.yaml" "envs/t1.py" "envs/__init__.py" "utils/shoulder_runner.py" "utils/model.py" "${URDF_FILE}" "${META_JSON}" "artifacts/autoresearch/run_kinematic_scratch_legopp300_progress.sh" "artifacts/autoresearch/run_shoulder4_eval_pair.sh" "artifacts/autoresearch/eval_shoulder4_frozen.py" "artifacts/autoresearch/export_shoulder_metrics.py"; then fail_run pack "tar packaging failed"; fi
phase_done pack
