...
def load_model(env, checkpoint, device, logstd_init, actor_mean_scale=None, logstd_min=None, logstd_max=None):
    model = ActorCritic(
        env.num_actions,
        env.num_obs,
...
def policy_step(env, model, obs):
    with torch.no_grad():
        full_action = torch.clamp(model.act(obs).loc, -1.0, 1.0)
        arm_action = full_action[:, env.arm_indices]
    return full_action, arm_action
...
def summarize_signed(values):
    ...
    return {
        "mean": float(sum(values) / len(values)),
        "p05": float(percentile(values, 0.05)),
...
def collect_sample(env, arm_action, cmd):
    ...
    return {
        ...
        "left_shoulder_pitch": left_pitch.detach().cpu().tolist(),
        "left_shoulder_roll": left_roll.detach().cpu().tolist(),
        "right_shoulder_pitch": right_pitch.detach().cpu().tolist(),
        "right_shoulder_roll": right_roll.detach().cpu().tolist(),
...
def summarize_command(env, model, name, cmd, steps, warmup_steps, gait_frequency):
    ...
    with torch.no_grad():
        for step in range(steps):
            full_action, arm_action = policy_step(env, model, obs)
    ...
    row = {
        ...
        "left_shoulder_pitch_mean": summarize_signed(flat("left_shoulder_pitch"))["mean"],
        "left_shoulder_pitch_p05": summarize_signed(flat("left_shoulder_pitch"))["p05"],
        "left_shoulder_pitch_p95": summarize_signed(flat("left_shoulder_pitch"))["p95"],
        "left_shoulder_roll_mean": summarize_signed(flat("left_shoulder_roll"))["mean"],
        "left_shoulder_roll_p05": summarize_signed(flat("left_shoulder_roll"))["p05"],
        "left_shoulder_roll_p95": summarize_signed(flat("left_shoulder_roll"))["p95"],
        "right_shoulder_pitch_mean": summarize_signed(flat("right_shoulder_pitch"))["mean"],
        "right_shoulder_pitch_p05": summarize_signed(flat("right_shoulder_pitch"))["p05"],
        "right_shoulder_pitch_p95": summarize_signed(flat("right_shoulder_pitch"))["p95"],
        "right_shoulder_roll_mean": summarize_signed(flat("right_shoulder_roll"))["mean"],
        "right_shoulder_roll_p05": summarize_signed(flat("right_shoulder_roll"))["p05"],
        "right_shoulder_roll_p95": summarize_signed(flat("right_shoulder_roll"))["p95"],
...
def random_replay(env, model, seconds, warmup_s, gait_frequency, max_speed, max_yaw, smooth_s, fps, checkpoint, seed):
    env.reset()
    ...
    with torch.no_grad():
        for step in range(total_steps):
            if step % progress_every == 0:
                print(f"random replay progress {step}/{total_steps} ({100.0 * step / max(1, total_steps):.0f}%)", flush=True)
            t = step * env.dt
    ...
    print(f"random replay progress {total_steps}/{total_steps} (100%)", flush=True)
    ...
    replay = {
        "title": "T1Shoulder4 NightH FullBody Scratch5000 随机命令长回放",
        "stage": "t1shoulder4_nighth_fullbody_random",
        "stageLabel": "T1Shoulder4 NightH FullBody Scratch5000 随机命令长回放",
...
def main():
    ...
    parser.add_argument("--task", default="T1Shoulder4NightHFullBodyGrid15Scratch_train5000")
    parser.add_argument("--checkpoint", required=True)
    parser.add_argument("--out_dir", default="artifacts/autoresearch/shoulder4_nighth_fullbody_grid15_scratch5000_eval")
    parser.add_argument("--label", default="shoulder4_nighth_fullbody_grid15_scratch5000")
    ...
    if args.mode in {"fixed", "both"}:
        ...
        for name, vx, vy, wz in command_suite():
            row = summarize_command(env, model, name, (vx, vy, wz), fixed_steps, warmup_steps, args.gait_frequency)
            rows.append(row)
            print(
