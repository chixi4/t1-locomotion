...
TASK="T1Shoulder4GaitPhaseNightN7PitchBoostSideAmpConservative_from7000LegFrozen_train300"
LABEL="shoulder4_night_n7_pitchboost_sideamp300"
DISPLAY_NAME="T1Shoulder4 NightN7 PitchBoost SideAmp model_300"
NOTE="Conservative side-lift-only tune based on N7P. shoulder_dynamic_roll_base_amp remains 0.0; pitch amp remains 0.84."
COMMAND_TEXT="Night N7 PitchBoost SideAmp Conservative：完全保留 N7P 的前后摆手和 roll 零基础，只把侧移动态外展 extra_amp 从 0.38 提到 0.46、roll_max 从 0.30 提到 0.34、dynamic_min_outward 从 0.08 提到 0.10。"
...
META_JSON="artifacts/autoresearch/nightn7_pitchboost_sideamp_runs.json"
...
echo "CONFIG side_amp extra=0.46 max=0.34 min_outward=0.10 base=0.0 pitch_amp=0.84" | tee -a "${TRAIN_LOG}"
...
if ! tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
  ...
  "${URDF_FILE}" \
  "${META_JSON}" \
  "artifacts/autoresearch/run_n7p_sideamp300_progress.sh" \
