...
PROC="$(ps -eo pid,etime,cmd | grep -E 'train_shoulder4_frozen.py.*KinematicScratchVelLegOpp|run_kinematic_scratch_vellegopp300_progress.sh' | grep -v grep | sed -n '1,3p')"
SCALARS=""
if [ -n "$LOG" ]; then
...
for tag in ['reward','steps','value_loss','kl_mean','lr','policy/mean_abs_max','curriculum/active_tilt_rms_mean','curriculum/active_arm_saturation_frac_mean','episode/shoulder_kinematic_motion_target','episode/base_height','episode/orientation']:
    if tag in tags:
        vals=ea.Scalars(tag)
        if vals:
            ev=vals[-1]
...
cat <<OUT
T1 Kinematic Scratch VelLegOpp Progress Dashboard
======================================================================
Run:   T1Shoulder4 Kinematic Scratch VelLegOpp model_300
Start: arm checkpoint null + frozen 7000 leg
Reward: no standing posture target, no low-speed threshold, no arm energy
Pitch: abs(cmd_x) * left-right foot x distance -> opposite shoulder pitch
Side:  outward foot velocity with signed delta lock: right foot outward raises left arm; left foot raises right
