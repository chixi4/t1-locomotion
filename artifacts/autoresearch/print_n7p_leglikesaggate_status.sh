...
CUR=0; TOTAL=300; ELAPSED="?"; ETA="?"; PCT="0.0"
if [ -n "$EPOCH" ]; then
...
PROC="$(ps -eo pid,etime,cmd | grep -E 'train_shoulder4_frozen.py.*LegLikeSagGate|run_n7p_leglikesaggate300_progress.sh' | grep -v grep | sed -n '1,3p')"
SCALARS=""
if [ -n "$LOG" ]; then
...
for tag in ['reward','steps','value_loss','kl_mean','lr','policy/mean_abs_max','curriculum/active_tilt_rms_mean','curriculum/active_arm_saturation_frac_mean','episode/shoulder_static_posture','episode/shoulder_dynamic_target','episode/shoulder_no_lazy_boundary','episode/shoulder_balance_smoothness']:
    if tag in tags:
        vals=ea.Scalars(tag)
        if vals:
            ev=vals[-1]
...
cat <<OUT
T1 LegLikeSagGate Progress Dashboard
======================================================================
Run:   T1Shoulder4 NightN7P SideAmp LegLikeSagGate model_300
Start: SideAmp model_300 + frozen 7000 leg, optimizer reset
Patch: dynamic_target -2.0 -> -4.0 | static residual *= (1 - pitch_gate)
Keep:  LegLikeClean 30s episode, command resample 8-12s, SideAmp lateral roll
Arm:   pitch_amp 0.84, roll_base 0.0, roll_extra 0.46, roll_max 0.34
