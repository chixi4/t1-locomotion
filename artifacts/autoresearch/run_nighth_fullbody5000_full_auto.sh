...
FINAL_ITER="${FINAL_ITER:-5000}"
TRAIN_ENVS="${TRAIN_ENVS:-30720}"
META_JSON="artifacts/autoresearch/nighth_fullbody_runs.json"
MASTER_LOG="artifacts/autoresearch/nighth_fullbody5000_full_auto_$(date +%Y%m%d_%H%M%S).log"
RUN_CODES=(scratch5000)
...
start_gpu_audit() {
  if [[ -x artifacts/autoresearch/nighth_fullbody_gpu_audit_wsl.sh ]]; then
    mkdir -p artifacts/autoresearch/gpu_train_audit
    if ! pgrep -f "nighth_fullbody_gpu_audit_wsl.sh" >/dev/null 2>&1; then
      nohup bash artifacts/autoresearch/nighth_fullbody_gpu_audit_wsl.sh \
        > artifacts/autoresearch/gpu_train_audit/nighth_fullbody_wsl_logger.out 2>&1 &
      echo "$!" > artifacts/autoresearch/gpu_train_audit/nighth_fullbody_wsl_logger.pid
      echo "==== GPU audit started pid=$(cat artifacts/autoresearch/gpu_train_audit/nighth_fullbody_wsl_logger.pid) ====" | tee -a "${MASTER_LOG}"
    else
...
if not candidates:
    raise SystemExit(f"no completed run dir found for {task} model_{final_iter}")
...
run_one() (
  ...
  RUN_ID="${TASK}_env${TRAIN_ENVS}_$(date +%Y%m%d_%H%M%S)_full_auto"
  ...
  echo "START run code=${CODE} label=${LABEL} task=${TASK} envs=${TRAIN_ENVS} final_iter=${FINAL_ITER} $(date '+%Y-%m-%d %H:%M:%S')" | tee "${TRAIN_LOG}"
  nvidia-smi --query-gpu=name,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits 2>/dev/null | tee -a "${TRAIN_LOG}" || true

  phase_start train
  stdbuf -oL -eL python3 artifacts/autoresearch/train_fullbody.py \
    --task "${TASK}" \
    --num_envs "${TRAIN_ENVS}" \
    --max_iterations "${FINAL_ITER}" \
    --run_name "${RUN_ID}" \
    --headless true 2>&1 | tee -a "${TRAIN_LOG}"
  phase_done

  RUN_DIR="$(find_run_dir)"
  CKPT="${RUN_DIR}/nn/model_${FINAL_ITER}.pth"
  URDF_FILE="$(asset_file)"
  echo "RUN_DIR=${RUN_DIR}" | tee -a "${TRAIN_LOG}"
  echo "CHECKPOINT=${CKPT}" | tee -a "${TRAIN_LOG}"
  ...
  tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
    ...
    "${URDF_FILE}" \
    "${META_JSON}" \
    "artifacts/autoresearch/train_fullbody.py" \
    "artifacts/autoresearch/eval_fullbody.py" \
    "artifacts/autoresearch/export_shoulder_metrics.py" \
    "artifacts/autoresearch/run_nighth_fullbody5000_full_auto.sh"
  phase_done

  python3 - "$LABEL" "$DISPLAY_NAME" "$TASK" "$RUN_DIR" "$CKPT" "$OUT_DIR" "$METRICS_JSON" "$WEB_TAR" "$TRAIN_LOG" "$TRAIN_ENVS" <<'PY'
import json
import sys
from datetime import datetime, timezone
from pathlib import Path
...
echo "==== NIGHTH FULLBODY START $(date '+%Y-%m-%d %H:%M:%S') order=${RUN_CODES[*]} envs=${TRAIN_ENVS} final_iter=${FINAL_ITER} ====" | tee -a "${MASTER_LOG}"
start_gpu_audit
...
echo "==== NIGHTH FULLBODY DONE $(date '+%Y-%m-%d %H:%M:%S') ====" | tee -a "${MASTER_LOG}"
