...
TASK="T1Shoulder4GaitPhaseSignFixAmp2StaticDown_from7000LegFrozen_train400"
LABEL="shoulder4_gaitphase_signfix_amp2_staticdown400"
DISPLAY_NAME="T1Shoulder4 SignFix Amp2StaticDown model_400"
...
python3 artifacts/autoresearch/export_shoulder_metrics.py \
  ...
  --checkpoint "$ARM_CKPT" \
  --note "Foot-phase sign -1.0, foot-phase weight -2.0, foot-phase amp 0.36, gait-phase pitch off, stationary-only same-down constraint." 2>&1 | tee -a "$TRAIN_LOG"
...
tar -czf "$WEB_TAR" \
  ...
  "$TRAIN_LOG" \
  "envs/${TASK}.yaml" \
  "artifacts/autoresearch/run_signfix_amp2_staticdown400_full_auto.sh" \
