...
TASK="T1Shoulder4KinematicScratchCrossSide_from7000LegFrozen_train300"
LABEL="shoulder4_kinematic_scratch_crossside300"
DISPLAY_NAME="T1Shoulder4 Kinematic Scratch CrossSide model_300"
NOTE="Scratch arm policy with one continuous kinematic motion reward: no explicit standing posture angle, no low-speed still threshold, cross-lateral side arm lift."
COMMAND_TEXT="KinematicScratchCrossSide: checkpoint=null; old shoulder style and arm energy rewards disabled; pitch target from abs(vx_cmd)*left-right foot x distance; side roll target cross-coupled so right leg outward raises left arm and left leg outward raises right arm."
...
META_JSON="artifacts/autoresearch/kinematic_scratch_crossside_runs.json"
...
echo "CONFIG reward: pitch abs(cmd_x)*foot_sag; cross-side roll right-leg->left-arm left-leg->right-arm" | tee -a "${TRAIN_LOG}"
phase_start train
...
if ! tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" "${OUT_DIR}" "${METRICS_JSON}" "${TRAIN_LOG}" "envs/${TASK}.yaml" "envs/t1.py" "envs/__init__.py" "utils/shoulder_runner.py" "utils/model.py" "${URDF_FILE}" "${META_JSON}" "artifacts/autoresearch/run_kinematic_scratch_crossside300_progress.sh" "artifacts/autoresearch/run_shoulder4_eval_pair.sh" "artifacts/autoresearch/eval_shoulder4_frozen.py" "artifacts/autoresearch/export_shoulder_metrics.py"; then fail_run pack "tar packaging failed"; fi
phase_done pack
