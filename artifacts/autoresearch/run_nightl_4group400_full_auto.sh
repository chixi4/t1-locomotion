...
FINAL_ITER="400"
META_JSON="artifacts/autoresearch/nightl_runs.json"
MASTER_LOG="artifacts/autoresearch/nightl_4group400_full_auto_$(date +%Y%m%d_%H%M%S).log"
RUN_CODES=(l)
  ...
  if [[ -x artifacts/autoresearch/nightl_gpu_audit_wsl.sh ]]; then
    ...
    if ! pgrep -f "nightl_gpu_audit_wsl.sh" >/dev/null 2>&1; then
      nohup bash artifacts/autoresearch/nightl_gpu_audit_wsl.sh \
        > artifacts/autoresearch/gpu_train_audit/nightl_wsl_logger.out 2>&1 &
      echo "$!" > artifacts/autoresearch/gpu_train_audit/nightl_wsl_logger.pid
      echo "==== GPU audit started pid=$(cat artifacts/autoresearch/gpu_train_audit/nightl_wsl_logger.pid) ====" | tee -a "${MASTER_LOG}"
    else
...
run_one() (
  ...
  tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
    ...
    "${URDF_FILE}" \
    "${META_JSON}" \
    "artifacts/autoresearch/run_nightl_4group400_full_auto.sh" \
...
echo "==== NIGHTL START $(date '+%Y-%m-%d %H:%M:%S') order=${RUN_CODES[*]} ====" | tee -a "${MASTER_LOG}"
start_gpu_audit
...
echo "==== NIGHTL DONE $(date '+%Y-%m-%d %H:%M:%S') ====" | tee -a "${MASTER_LOG}"
