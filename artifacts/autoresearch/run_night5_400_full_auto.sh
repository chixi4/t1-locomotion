...
FINAL_ITER="400"
META_JSON="artifacts/autoresearch/night5_runs.json"
MASTER_LOG="artifacts/autoresearch/night5_400_full_auto_$(date +%Y%m%d_%H%M%S).log"
RUN_CODES=(b c a e d)
...
start_gpu_audit() {
  if [[ -x artifacts/autoresearch/night5_gpu_audit_wsl.sh ]]; then
    mkdir -p artifacts/autoresearch/gpu_train_audit
    if ! pgrep -f "night5_gpu_audit_wsl.sh" >/dev/null 2>&1; then
      nohup bash artifacts/autoresearch/night5_gpu_audit_wsl.sh \
        > artifacts/autoresearch/gpu_train_audit/night5_wsl_logger.out 2>&1 &
      echo "$!" > artifacts/autoresearch/gpu_train_audit/night5_wsl_logger.pid
      echo "==== GPU audit started pid=$(cat artifacts/autoresearch/gpu_train_audit/night5_wsl_logger.pid) ====" | tee -a "${MASTER_LOG}"
    else
...
run_one() (
  ...
  tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
    ...
    "${URDF_FILE}" \
    "${META_JSON}" \
    "artifacts/autoresearch/run_night5_400_full_auto.sh" \
...
echo "==== NIGHT5 START $(date '+%Y-%m-%d %H:%M:%S') order=${RUN_CODES[*]} ====" | tee -a "${MASTER_LOG}"
start_gpu_audit
...
echo "==== NIGHT5 DONE $(date '+%Y-%m-%d %H:%M:%S') ====" | tee -a "${MASTER_LOG}"
