...
TASK="T1Shoulder4GaitPhaseSymFixFoot2Norm_from7000LegFrozen_train400"
LABEL="shoulder4_gaitphase_symfix_foot2_norm400"
DISPLAY_NAME="T1Shoulder4 SymFix Foot2Norm model_400"
...
for cfg_path in Path("logs").glob("*/config.yaml"):
    try:
        ...
        continue
    if cfg.get("basic", {}).get("task") == task and (cfg_path.parent / "nn" / "model_400.pth").exists():
...
ARM_CKPT="${RUN_DIR}/nn/model_400.pth"
...
python3 artifacts/autoresearch/export_shoulder_metrics.py \
  ...
  --checkpoint "$ARM_CKPT" \
  --note "SymFix foot-phase pitch weight -2.0 and normalized shoulder pair symmetry." 2>&1 | tee -a "$TRAIN_LOG"
...
tar -czf "$WEB_TAR" \
  ...
  "$TRAIN_LOG" \
  "envs/${TASK}.yaml" \
  "artifacts/autoresearch/run_symfix_foot2_norm400_full_auto.sh" \
