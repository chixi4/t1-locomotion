...
def main() -> None:
    runs = json.loads(Path("artifacts/autoresearch/nightk_runs.json").read_text(encoding="utf-8"))
    for run in runs:
        ...
        assert cfg["rewards"]["shoulder_lateral_roll_amp"] >= 0.04
        assert cfg["rewards"]["shoulder_lateral_roll_extra_amp"] >= 0.25
        assert cfg["rewards"]["shoulder_lateral_roll_max"] <= 0.24
        assert cfg["rewards"]["shoulder_lateral_roll_min_outward"] >= 0.02
        assert cfg["rewards"]["scales"]["shoulder_lateral_roll_wrong_sign_action"] < 0.0
