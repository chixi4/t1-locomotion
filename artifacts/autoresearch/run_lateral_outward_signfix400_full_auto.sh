...
LABEL="shoulder4_gaitphase_lateral_outward_signfix400"
DISPLAY_NAME="T1Shoulder4 LateralOutward SignFix model_400"
...
python3 artifacts/autoresearch/export_shoulder_metrics.py \
  ...
  --checkpoint "${ARM_CKPT}" \
  --note "Lateral outward sign-fix arm design: sagittal foot-x pitch swing only for vx/yaw, lateral roll outward with left_sign=+1 right_sign=-1, amp 0.22, foot-extra 0.18, roll max 0.48, roll soft limit 0.55, and lateral pitch-down same-angle constraint." 2>&1 | tee -a "${TRAIN_LOG}"
phase_done
...
tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
  ...
  "artifacts/autoresearch/run_lateral_outward_signfix400_full_auto.sh" \
