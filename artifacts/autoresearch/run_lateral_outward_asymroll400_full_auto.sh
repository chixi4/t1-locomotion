...
TASK="T1Shoulder4GaitPhaseLateralOutwardAsymRollLimit_from7000LegFrozen_train400"
LABEL="shoulder4_gaitphase_lateral_outward_asymroll400"
DISPLAY_NAME="T1Shoulder4 LateralOutward AsymRoll model_400"
...
python3 artifacts/autoresearch/export_shoulder_metrics.py \
  ...
  --checkpoint "${ARM_CKPT}" \
  --note "Lateral outward asym-roll arm design: sagittal foot-x pitch swing only for vx/yaw, lateral roll outward with left_sign=+1 right_sign=-1, amp 0.22, foot-extra 0.18, roll max 0.48, inward roll target/hard limit capped at 0.14 rad while outward remains 0.55 rad, and lateral pitch-down same-angle constraint." 2>&1 | tee -a "${TRAIN_LOG}"
phase_done
...
tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
  ...
  "utils/model.py" \
  "resources/T1/T1_locomotion_shoulder4_lateral_roll_asym_limit.urdf" \
  "artifacts/autoresearch/run_lateral_outward_asymroll400_full_auto.sh" \
