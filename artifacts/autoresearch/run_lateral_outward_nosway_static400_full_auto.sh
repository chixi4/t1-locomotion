...
TASK="T1Shoulder4GaitPhaseLateralOutwardNoSwayStatic_from7000LegFrozen_train400"
LABEL="shoulder4_gaitphase_lateral_outward_nosway_static400"
DISPLAY_NAME="T1Shoulder4 LateralOutward NoSwayStatic model_400"
...
python3 artifacts/autoresearch/export_shoulder_metrics.py \
  ...
  --checkpoint "${ARM_CKPT}" \
  --note "NoSwayStatic source-fix arm design: anti_sway_vs_fixed_arm disabled, true still-command training proportion 0.10, sagittal foot-x pitch swing only for vx/yaw, lateral roll outward with left_sign=+1 right_sign=-1, base amp 0.09, foot-side extra 0.20, roll max 0.30, asymmetric roll target/hard limits inward 0.14 rad and outward 0.30 rad, and neutral same-angle pitch constraints for still/lateral commands. Eval JSON includes signed shoulder pitch/roll means and p05/p95." 2>&1 | tee -a "${TRAIN_LOG}"
phase_done
...
tar "${TAR_COMPRESS[@]}" -cf "${WEB_TAR}" \
  ...
  "artifacts/autoresearch/run_lateral_outward_nosway_static400_full_auto.sh" \
