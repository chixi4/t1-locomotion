...
def main() -> None:
    runs = json.loads(Path("artifacts/autoresearch/nighti_runs.json").read_text(encoding="utf-8"))
    for run in runs:
        ...
        assert cfg["rewards"]["shoulder_lateral_roll_amp"] <= 0.065
        assert cfg["rewards"]["shoulder_lateral_roll_extra_amp"] >= 0.28
        assert cfg["rewards"]["shoulder_lateral_roll_min_outward"] <= 0.04
