...
CUR=0; TOTAL=500; ELAPSED="?"; ETA="?"; PCT="0.0"
if [ -n "$EPOCH" ]; then
...
PROC="$(ps -eo pid,etime,cmd | grep -E 'train_shoulder4_frozen.py.*OfficialStrongZeroVelOpp|run_official_strongzero_velopp500_progress.sh' | grep -v grep | sed -n '1,3p')"
SCALARS=""
if [ -n "$LOG" ]; then
...
cat <<OUT
T1 Official StrongZero VelOpp Progress Dashboard
======================================================================
Run:   T1Shoulder4 Official StrongZero VelOpp model_500
Start: arm checkpoint null + frozen 7000 leg
Reward: official leg settings + stronger zero standing arm target
