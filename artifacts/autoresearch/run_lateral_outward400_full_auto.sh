...
TASK="T1Shoulder4GaitPhaseLateralOutward_from7000LegFrozen_train400"
LABEL="shoulder4_gaitphase_lateral_outward400"
DISPLAY_NAME="T1Shoulder4 LateralOutward model_400"
...
python3 artifacts/autoresearch/export_shoulder_metrics.py \
  ...
  --checkpoint "${ARM_CKPT}" \
  --note "Lateral outward arm design: sagittal foot-x pitch swing only for vx/yaw, lateral roll outward with amp 0.22, foot-extra 0.18, roll max 0.48, roll soft limit 0.55, and lateral pitch-down same-angle constraint." 2>&1 | tee -a "${TRAIN_LOG}"
...
tar -czf "${WEB_TAR}" \
  ...
  "utils/model.py" \
  "resources/T1/T1_locomotion_shoulder4_lateral_roll_softlimit.urdf" \
  "artifacts/autoresearch/run_lateral_outward400_full_auto.sh" \
