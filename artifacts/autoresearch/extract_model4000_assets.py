...
RUN = "logs/2026-05-05-11-09-07"
LABEL = "T1CircleGridFace6Tight15 model_4000"
CKPT = f"{RUN}/nn/model_4000.pth"
...
def extract_metrics(accumulator, tags):
    ...
    path = "artifacts/autoresearch/t1gridface6_15_model_4000_metrics.json"
    with open(path, "w", encoding="utf-8") as file:
        json.dump(output, file, ensure_ascii=False, separators=(",", ":"))
    return path, summary


def extract_curriculum(accumulator, tags, cfg):
    wanted_tags = [
        "value_loss",
        "kl_mean",
        "lr",
    ...
    output = {
        ...
        "grid": {
            "linLevels": int(commands["lin_vel_levels"]),
            "yawLevels": int(commands["ang_vel_levels"]),
            "shape": checkpoints[-1]["shape"] if checkpoints else [0, 0, 0],
            "linearSpeedRange": commands.get("linear_speed", [0.0, 1.5]),
            "yawRangeApprox": commands.get("ang_vel_yaw", [-1.5, 1.5]),
            "unlockNeighbors": commands.get("grid3d_unlock_neighbors", "face6"),
    ...
    path = "artifacts/autoresearch/t1gridface6_15_model4000_curriculum_unlock_timeline.json"
    with open(path, "w", encoding="utf-8") as file:
        json.dump(output, file, ensure_ascii=False, separators=(",", ":"))
...
def main():
    os.chdir(ROOT)
    with open(os.path.join("envs", f"{TASK}.yaml"), "r", encoding="utf-8") as file:
    ...
    curriculum_path, curriculum_summary = extract_curriculum(accumulator, tags, cfg)
    print(json.dumps({
